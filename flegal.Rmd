...
# if distribution is UNIF, Eparam1 = E min, Eparam2 = E max
# if distribution is NORM, Eparam1 = E mean, Eparam2 = standard deviation of exposure
simulate_flegalV3 <- function(N, distrib, Eparam1, Eparam2, cutoff, a, b, MEsigma, Nsims){
  E <- c()
  D <- c()
  Ehigh <- c()
  Eerror <- c()
...
simulate_flegalV3(1000, "NORM", 2050, 150, 2200, -4.5, 0.0019, newvar, 100)

simulate_flegalV3(1000, "EVEN", 1600, 2499, 2200, -4.5, 0.0019, newvar, 100)

testcase2 <- simulate_flegal(1000, "NORM", 2050, 150, 2200, -4.5, 0.0019, newvar, 400)

```

Okay so I'm still getting different Se, Sp from flegal case. Trying to re-examine how I get the numbers.
```{r createfunctions} 
# if distribution is UNIF, Eparam1 = E min, Eparam2 = E max
# if distribution is NORM, Eparam1 = E mean, Eparam2 = standard deviation of exposure
simulate_flegalV3 <- function(N, distrib, Eparam1, Eparam2, cutoff, a, b, MEsigma, Nsims){
  ...
  for (j in 1:Nsims){
    ...
    for (i in seq_along(MEsigma)){
      fakepop[[i]] <- tibble(E = E, Ehigh = ifelse(E>2200, 1, 0), D = D, 
                  Eerror = rnorm(N, mean = 0, sd = MEsigma), Eprime = E + Eerror,
                  Eprimehigh = ifelse(Eprime > cutoff, 1, 0))
      popsum[[i]] <- fakepop[[i]] %>% group_by(D, Ehigh, Eprimehigh) %>% summarise(n=n(), .groups = "keep") 
      
      values[i,1] <- MEsigma[[i]] #this will be constant but want it to pass through
      values[i,2] <- (popsum[4,4] + popsum[8,4])/ (popsum[3,4] + popsum[4,4] + popsum[7,4] + popsum[8,4]) #Se all
      values[i,3] <- (popsum[4,4])/ (popsum[3,4] + popsum[4,4]) #Se D=1
      values[i,4] <- (popsum[8,4])/ (popsum[7,4] + popsum[8,4])#Se D=0
      values[i,5] <- (popsum[1,4] + popsum[5,4])/ (popsum[1,4] + popsum[2,4] + popsum[5,4] + popsum[6,4]) #Sp all
      values[i,6] <- (popsum[1,4])/ (popsum[1,4] + popsum[2,4]) #Sp D=1
      values[i,7] <- (popsum[5,4])/ (popsum[5,4] + popsum[6,4])  #Sp D=0
      values[i,8] <- (popsum[7,4] + popsum[8,4]) / (popsum[3,4] + popsum[4,4] + popsum[7,4] + popsum[8,4]) #RR
      values[i,9] <- (popsum[6,4] + popsum[8,4]) / (popsum[2,4] + popsum[4,4] + popsum[6,4] + popsum[8,4]) #RR measured
...
simulate_flegalV3(100, "EVEN", 2050, 150, 2200, -4.5, 0.0019, newvar, 200)
...
case1A <- simulate_flegal(100, "UNIF", 1600, 2499, 2200, -4.5, 0.0019, MEsigma, 200, 10)
case1B <- simulate_flegal(100, "UNIF", 1600, 2499, 2200, -8.0, 0.0035, MEsigma, 200, 10)
case1C <- simulate_flegal(100, "UNIF", 1600, 2499, 2200, -16.0, 0.0072, MEsigma, 200, 10)

case1 <- rbind(case1A, case1B, case1C)
case1
```

*Case 2* Repeating the same but with 1000 simulations. Everything else is same.  
```{r, case2}
set.seed(303)
case2A <- simulate_flegal(100, "UNIF", 1600, 2499, 2200, -4.5, 0.0019, MEsigma, 1000, 100)
case2B <- simulate_flegal(100, "UNIF", 1600, 2499, 2200, -8.0, 0.0035, MEsigma, 1000, 100)
case2C <- simulate_flegal(100, "UNIF",1600, 2499, 2200, -16.0, 0.0072, MEsigma, 1000, 100)
...
set.seed(303)
case3A <- simulate_flegal(1000, "UNIF", 1600, 2499, 2200, -4.5, 0.0019, MEsigma, 1000, 100)
case3B <- simulate_flegal(1000, "UNIF", 1600, 2499, 2200, -8.0, 0.0035, MEsigma, 1000, 100)
case3C <- simulate_flegal(1000, "UNIF", 1600, 2499, 2200, -16.0, 0.0072, MEsigma, 1000, 100)
...
```{r case4, eval = F}
set.seed(303)
case4A <- simulate_flegal(100000, "UNIF", 1600, 2499, 2200, -4.5, 0.0019, MEsigma, 1000, 100)
case4B <- simulate_flegal(100000, "UNIF", 1600, 2499, 2200, -8.0, 0.0035, MEsigma, 1000, 100)
case4C <- simulate_flegal(100000, "UNIF", 1600, 2499, 2200, -16.0, 0.0072, MEsigma, 1000, 100)
...
*Case 5* Changing the exposure distribution to centered about midpoint ~N(2050, 150)   
*Case 6* Exposure distribution centered about cutpoint  
...
set.seed(303)
case5A <- simulate_flegal(1000, "NORM", 2050, 150, 2200, -4.5, 0.0019, MEsigma, 1000, 100)
case5B <- simulate_flegal(1000, "NORM", 2050, 150, 2200, -8.0, 0.0035, MEsigma, 1000, 100)
case5C <- simulate_flegal(1000, "NORM", 2050, 150, 2200, -16.0, 0.0072, MEsigma, 1000, 100)
...
set.seed(303)
case6A <- simulate_flegal(1000, "NORM", 2200, 150, 2200, -4.5, 0.0019, MEsigma, 1000, 100)
case6B <- simulate_flegal(1000, "NORM", 2200, 150, 2200, -8.0, 0.0035, MEsigma, 1000, 100)
case6C <- simulate_flegal(1000, "NORM", 2200, 150, 2200, -16.0, 0.0072, MEsigma, 1000, 100)
case6 <- rbind(case6A, case6B, case6C)

set.seed(303)
case7A <- simulate_flegal(1000, "NORM", 1900, 150, 2200, -4.5, 0.0019, MEsigma, 1000, 100)
case7B <- simulate_flegal(1000, "NORM", 1900, 150, 2200, -8.0, 0.0035, MEsigma, 1000, 100)
case7C <- simulate_flegal(1000, "NORM", 1900, 150, 2200, -16.0, 0.0072, MEsigma, 1000, 100)
...
```{r huge, eval = FALSE}
set.seed(303)
case8A <- simulate_flegal(100000, "NORM", 2050, 150, 2200, -4.5, 0.0019, MEsigma, 1000)
case8B <- simulate_flegal(100000, "NORM", 2050, 150, 2200, -8.0, 0.0035, MEsigma, 1000)
case8C <- simulate_flegal(100000, "NORM", 2050, 150, 2200, -16.0, 0.0072, MEsigma, 1000)
...
all_cases <- rbind(case1, case2, case3, case5, case6, case7)
write.csv(all_cases, "flegal_export_07Nov2022.csv")
```
