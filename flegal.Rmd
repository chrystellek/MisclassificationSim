...
The first time I made this function, disease was being assigned anew each simulation except for exposure. To reduce variability, created function that will fix the exposure AND disease status before measurement error is simulated. Note though that Flegal et al. did not fix disease before simulations, since they present averages of the "true" and "observed" RRs (Table 3).  

This version is meant for one value of the SD of ME (removed some of the arguments to make it easier to test)
```{r createsim, eval = FALSE}
...
# i know it's not good variable naming but it's just for testing
...
set.seed(10)
testcase1A <- flegal_sim_fixed(100, 1600, 2499, 2200, -4.5, 0.0019, MEsigma, 200)
```
