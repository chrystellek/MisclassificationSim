...
```{r createfunctions}
# this function does the exposure misclassification
# I couldn't figure out how to make it all one function 
...
simulate_flegal <- function(N, distrib, Eparam1, Eparam2, cutoff, a, b, MEsigma, Nsims, Ndisease){
    output <- matrix(nrow = length(MEsigma), ncol = 18, 
                   dimnames = list(c(),c("N", "Distrib", "Eparam1", "Eparam2", "cutoff", "a", "b", "Nsims", "Ndisease", "ME", 
  ...
  output <- as.data.frame(cbind(N, distrib, Eparam1, Eparam2, cutoff, a, b, Nsims, Ndisease, all_means))
  return(output)
}

newvar <- c(100, 300, 500)

testcase <- simulate_flegal(1000, "NORM", 2050, 150, 2200, -4.5, 0.0019, MEsigma, 100)
