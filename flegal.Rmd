...
```{r flegalfunctions}
# These are with fixed exposure and disease before simulations
...
normal_sim_fixed <- function(N, Emean, Esigma, cutpt, a, b, MEsigma, Nsims){
  ...
  E <- as.integer(rnorm(N, mean = Emean, sd = Esigma))
  D <- rbinom(n = N, size = 1, p = plogis(a + b*E))
...
## Another attempt
Decision to make: fix E distribution and try many fixed D, or many sims of E and D up top but E can also change? 
```{r flegalfunctions2, eval = F}
# trying to iterate through fixed disease for each ME sim, but simulate across different disease assignments as well
flegal_fixed <- function(N, Emin, Emax, cutpt, a, b, MEsigma, Nsims){
  intermedresults <- matrix(nrow = length(Nsims), ncol = 16)
  allresults <- matrix(nrow = length(MEsigma), ncol = 16, dimnames = list(c(),c("N", "Emin", "Emax", "cutoff", "a", "b", "Nsims", "ME", "SeAll", "SeD1", "SeD0", "SpAll", "SpD1", "SpD0", "RRtrue", "RRobs")))
  ...
  for (k in 1:Nsims){
    ...
    for (j in seq_along(MEsigma)){
      ...
      # "N", "Emin", "Emax", "cutoff", "a", "b", "Nsims", "ME", "SeAll", "SeD1", "SeD0", "SpAll", "SpD1", "SpD0", "RRtrue", "RRobs"
      results[j,] <- cbind(N, Emin, Emax, cutpt, a, b, Nsims, MEsigma[[j]], mean(values[,1]), mean(values[,2]), mean(values[,3]), mean(values[,4]), mean(values[,5]), mean(values[,6]), mean(values[,7]), mean(values[,8]))
    # creates matrix with 3 rows  
    }
    # do I need to first create matrix to store all results: so nrows = Nsims, then bind those 
    # want to create a matrix where I define rows 1, 2, 3 
    # i have not run this - I am confused. 
    intermedresults[k,] <- cbind(N, Emin, Emax, cutpt, a, b, Nsims, results[1,8], results[1,9], results[1,10], results[1,11], results[1,12], results[1,13], results[1,14], results[1,15], results[1,16])
    intermedresults[2,] <- cbind(N, Emin, Emax, cutpt, a, b, Nsims, results[2,8], results[2,9], results[2,10], results[2,11], results[2,12], results[2,13], results[2,14], results[2,15], results[2,16])
    intermedresults[3,] <- cbind(N, Emin, Emax, cutpt, a, b, Nsims, results[3,8], results[3,9], results[3,10], results[3,11], results[3,12], results[3,13], results[3,14], results[3,15], results[3,16])
   
     allresults[1,] <- cbind(N, Emin, Emax, cutpt, a, b, Nsims, mean(intermedresults[1,8]), mean(intermedresults[1,9]), mean(intermedresults[1,10]), mean(intermedresults[1,11]), mean(intermedresults[1,12]), mean(intermedresults[1,13]), mean(intermedresults[1,14]), mean(intermedresults[1,15]), mean(intermedresults[1,16]))
  }
  allresults <- as.data.frame(allresults)
  return(allresults)
}
orange <- c(100, 300, 500)
test1 <- flegal_fixed(100, 1600, 2499, 2200, -4.5, 0.0019, MEsigma, 200)
```
...
*Case 6* Exposure distribution centered about midpoint  
