...
Want to create scatter plots for each distribution.
```{r scatterplots1}
# A. a = -4.5, b = 0.0019 
pop1 <- tibble(
...
This is the first working one that does not have disease fixed - used to create results shared Aug 10 with group
```{r function}
...
The first time I made this, disease was being assigned anew each simulation except for exposure. To reduce variablity, working on a function that will fix the exposure AND disease status before measurement error is simulated. My current problem is that I think I need to also run through the different measurement error SD values in the same function, in order to keep the disease status consistent across examples. Note though that Flegal et al. did not fix disease before simulations, since they present averages of the "true" and "observed" RRs (Table 3).  

This version is meant for one value of the SD of ME (removed some of the arguments to make it easier to test)
```{r createsim}
flegal_sim1 <- function(N, Emin, Emax, cutpt, a, b, MEsigma, Nsims){
 # values <- matrix(nrow = length(Nsims), ncol = 8, dimnames = list(c(),c("E", "D", "Eerror", "Eprime", "Ehigh", "Eprimehigh")))
  values <- matrix(nrow = Nsims, ncol = 8, dimnames = list(c(), c("Se", "SeD1", "SeD0", "Sp", "SpD1", "SpD0", "RRtrue", "RRobs")))
  results <- matrix(nrow = 1, ncol = 11, dimnames = list(c(), c("N", "Nsims", "ME", "Se", "SeD1", "SeD0", "Sp", "SpD1", "SpD0", "RRtrue", "RRobs")))
 # results <- data.frame()
 ...
 # for (j in seq_along(MEsigma)){
    ...
    for (i in 1:Nsims){
      # want to store these then afterwards take the means or something
      Eerror <- rnorm(N, mean = 0, sd = MEsigma)
      Eprime <- E + Eerror
      Ehigh <- ifelse(E > cutpt, 1, 0)
      Eprimehigh <- ifelse(Eprime > cutpt, 1, 0)
    
    # creating a new matrix based on those values. These are not Flegal formulas?
  ...
  results <- cbind(N, Nsims, MEsigma, mean(values[,1]), mean(values[,2]), mean(values[,3]), mean(values[,4]), mean(values[,5]), mean(values[,6]), mean(values[,7]), mean(values[,8]))
  
  #}
    #(N, Emin, Emax, cutpt, a, b, Nsims, MEsigma, mean(Se_all), mean(Se_D1), mean(Se_D0), mean(Sp_all), mean(Sp_D1), mean(Sp_D0), mean(RR_true), mean(RR_obs))  
 # all_results <- rbind(allresults, results)
...
pepper <- flegal_sim1(100, 1600, 2499, 2200, -4.5, 0.0019, 100, 200)
pepper

trial2 <- data.frame(matrix(nrow = 3, ncol = 11))

for (j in seq_along(ME_SD)){
  trial2[j,] <- flegal_onesim(100, 1600, 2499, 2200, -4.5, 0.0019, ME_SD[[j]], 200)
  #try_all <- rbind(try_all, try[j])
}
View(trial2)
```

Some notes about my various attempts:  
- using apply seems inappropriate because if I apply the single simulation to a vector of ME values, the disease status will change for each  


This seems to work for all three...
...
```{r flegalfunction}
#function input MEsigma can be a vector
flegal_sim_fixed <- function(N, Emin, Emax, cutpt, a, b, MEsigma, Nsims){
 # values <- matrix(nrow = length(Nsims), ncol = 8, dimnames = list(c(),c("E", "D", "Eerror", "Eprime", "Ehigh", "Eprimehigh")))
  values <- matrix(nrow = Nsims, ncol = 8, dimnames = list(c(),c("Se", "SeD1", "SeD0", "Sp", "SpD1", "SpD0", "RRtrue", "RRobs")))
  results <- as.data.frame(matrix(nrow = length(MEsigma), ncol = 11, dimnames = list(c(),c("N", "Nsims", "ME", "Se", "SeD1", "SeD0", "Sp", "SpD1", "SpD0", "RRtrue", "RRobs"))))
 # results <- data.frame()
   allresults <- data.frame()
  ...
  for (j in seq_along(MEsigma)){
    ...
    # could maybe use summarise instead?
    #single bracket refers to location inside, double is more for iteration?
  results[j,] <- cbind(N, Nsims, MEsigma[[j]], mean(values[,1]), mean(values[,2]), mean(values[,3]), mean(values[,4]), mean(values[,5]), mean(values[,6]), mean(values[,7]), mean(values[,8]))
...
## function seems to loop correctly but different answers than flegal
orange <- c(100, 300, 500)
test1 <- flegal_sim_fixed(100, 1600, 2499, 2200, -4.5, 0.0019, orange, 200)
test1

   # these are Flegal's formulas... they don't make sense to me. Creates values >1?
      # values[i,1] <- sum(Eprimehigh==1)/sum(Ehigh==1)
      # values[i,2] <- sum(Eprimehigh[D==1]==1)/sum(Ehigh[D==1]==1)
      # values[i,3] <- sum(Eprimehigh[D==0]==1)/sum(Ehigh[D==0]==1)
      # 
      # values[i,4] <- sum(Eprimehigh==0)/sum(Ehigh==0)
      # values[i,5] <- sum(Eprimehigh[D==1]==0)/sum(Ehigh[D==1]==0)
      # values[i,6] <- sum(Eprimehigh[D==0]==0)/sum(Ehigh[D==0]==0)
      # 
      # values[i,7] <- (sum(D==1 & Ehigh==1)/sum(Ehigh==1))/(sum(D==1 & Ehigh==0)/sum(Ehigh==0))
      # values[i,8] <- (sum(D==1 & Eprimehigh==1)/sum(Eprimehigh==1))/(sum(D==1 & Eprimehigh==0)/sum(Eprimehigh==0))

```
Okay... I am a little concerned about how close the values are together. I have tried this approach outside of the function and it seemed to work. AND actually this makes sense because disease is fixed. 
...
```
The RR for the first case is far from expected 1.66
...
case2
```
When we increase the simulations, some of the results are closer to original. Not sure what is going on with RR's 
...
uni_sim <- function(N, Emin, Emax, cutpt, a, b, MEsigma, Nsims){
  ...
  RR_true <- c()
  RR_obs <- c()
  results <- as.data.frame(matrix(nrow = length(MEsigma), ncol = 4) ) #, dimnames = list(c(),c("N", "Nsims", "ME", "Se", "SeD1", "SeD0", "Sp", "SpD1", "SpD0", "RRtrue", "RRobs"))))
  ...
  for (j in seq_along(MEsigma)){
    ...
    results[j,] <- cbind(N, Nsims, MEsigma[[j]], mean(Se_all), mean(Se_D1), mean(Se_D0), mean(Sp_all), mean(Sp_D1), mean(Sp_D0), mean(RR_true), mean(RR_obs))
    }
 # params <- data.frame(N, Emin, Emax, cutpt, a, b, Nsims, MEsigma, mean(Se_all[[j]]), mean(Se_D1), mean(Se_D0), mean(Sp_all), mean(Sp_D1), mean(Sp_D0), mean(RR_true), mean(RR_obs))  
 # colnames(params) <- c("N", "Emin", "Emax", "cutoff", "a", "b", "sims", "SD", "Se_all", "Se_D1", "Se_D0", "Sp_all", "Sp_D1", "Sp_D0", "RR_true", "RR_obs")
  return(results)
...
ucase3A <- uni_sim(1000, 1600, 2499, 2200, -4.5, 0.0019, MEsigma, 1000)
ucase3B <- uni_sim(1000, 1600, 2499, 2200, -8.0, 0.0035, MEsigma, 1000)
ucase3C <- uni_sim(1000, 1600, 2499, 2200, -16.0, 0.0072, MEsigma, 1000)

ucase3 <- rbind(ucase3A, ucase3B, ucase3C)

comp3 <- cbind(case3, ucase3) #comparing how the seq vs. uniform functions do 
comp3 
```
...
```{r case4}
set.seed(303)
case4A <- uni_sim(10000, 1600, 2499, 2200, -4.5, 0.0019, MEsigma, 1000)
case4B <- uni_sim(10000, 1600, 2499, 2200, -8.0, 0.0035, MEsigma, 1000)
case4C <- uni_sim(10000, 1600, 2499, 2200, -16.0, 0.0072, MEsigma, 1000)
...
```{r createnorm}
normal_sim <- function(N, Emean, Esigma, cutoff, a, b, MEsigma, Nsims){
# the flegal sim attempted to do this more elegantly
  Se_all <- c()
  Se_D1 <- c()
  Se_D0 <- c()
  Sp_all <- c()
  ...
  RR_true <- c()
  RR_obs <- c()
  params <- matrix(nrow = length(MEsigma), ncol = 16, dimnames = list(c(),c("N", "Emean", "ESD", "cutoff", "a", "b", "sims", "SD", "Se_all", "Se_D1", "Se_D0", "Sp_all", "Sp_D1", "Sp_D0", "RR_true", "RR_obs")))
...
for (j in seq_along(MEsigma)){
  ...
  ########## FIX THIS
    params[j,] <- cbind(N, Emean, Esigma, cutoff, a, b, Nsims, MEsigma[[j]], mean(Se_all), mean(Se_D1), mean(Se_D0), mean(Sp_all), mean(Sp_D1), mean(Sp_D0), mean(RR_true), mean(RR_obs))
...
case5A <- normal_sim(1000, 2050, 150, 2200, -4.5, 0.0019, MEsigma, 1000)
case5B <- normal_sim(1000, 2050, 150, 2200, -8.0, 0.0035, MEsigma, 1000)
case5C <- normal_sim(1000, 2050, 150, 2200, -16.0, 0.0072, MEsigma, 1000)
